...
public_remote_key = (
    "https://b2.tethys-ts.xyz/file/tethysts/tethys/public_remotes_v4.json.zst"
)

local_results_name = (
    "{ds_id}/{version_date}/{stn_id}/{chunk_id}.{chunk_hash}.results.h5"
)
...
def make_run_date_key(run_date=None):
    ...
    if run_date is None:
        ...
        raise TypeError(
            "run_date must be None, Timestamp, or a string representation of a timestamp"
        )
...
def create_public_s3_url(base_url, bucket, obj_key):
    ...
    if "contabo" in base_url:
        key = contabo_public_key_pattern.format(
            base_url=base_url.rstrip("/"), bucket=bucket, obj_key=obj_key
        )
    else:
        key = b2_public_key_pattern.format(
            base_url=base_url.rstrip("/"), bucket=bucket, obj_key=obj_key
        )
...
def chunk_filters(
    ...
    ## Temporal filters
    if isinstance(from_date, (str, pd.Timestamp, datetime)) and (
        "chunk_day" in first_one
    ):
    ...
    if isinstance(to_date, (str, pd.Timestamp, datetime)) and (
        "chunk_day" in first_one
    ):
    ...
    if isinstance(from_mod_date, (str, pd.Timestamp, datetime)) and (
        "modified_date" in first_one
    ):
    ...
    if isinstance(to_mod_date, (str, pd.Timestamp, datetime)) and (
        "modified_date" in first_one
    ):
...
def download_results(
    chunk: dict,
    bucket: str,
    s3: botocore.client.BaseClient = None,
    connection_config: dict = None,
    public_url: HttpUrl = None,
    cache: Union[pathlib.Path] = None,
    from_date=None,
    to_date=None,
    return_raw=False,
):
    """ """
    file_obj = s3tethys.get_object_s3(
        chunk["key"], bucket, s3, connection_config, public_url
    )

    if isinstance(cache, pathlib.Path):
        chunk_hash = chunk["chunk_hash"]
        version_date = pd.Timestamp(chunk["version_date"]).strftime("%Y%m%d%H%M%SZ")
        results_file_name = local_results_name.format(
            ds_id=chunk["dataset_id"],
        ...
        if not chunk_path.exists():
            ...
            if chunk["key"].endswith(".zst"):
                data = xr.load_dataset(
                    s3tethys.decompress_stream_to_object(
                        io.BytesIO(file_obj.read()), "zstd"
                    )
                )
                H5(data).sel(exclude_coords=["station_geometry", "chunk_date"]).to_hdf5(
                    chunk_path, compression="zstd"
                )
                data.close()
                del data
            else:
        ...
        if chunk["key"].endswith(".zst"):
            file_obj = s3tethys.decompress_stream_to_object(
                io.BytesIO(file_obj.read()), "zstd"
            )
        ...
        for var in chunk.data_vars:
            if var not in chunk_dict:
                ...
                var_dict = {
                    ...
                    "enc": enc,
                    "dtype": dtype,
                    "attrs": chunk[var].attrs,
                }
                chunk_dict[var] = var_dict

    try:
        xr3 = xr.combine_by_coords(
            coords_list,
            compat="override",
            data_vars="minimal",
            coords="all",
    ...
    for chunk in datasets:
        for var in chunk.data_vars:
            if isinstance(chunk[var].variable._data, np.ndarray):
                xr3[var].loc[
                    chunk[var].transpose(*chunk_dict[var]["dims"]).coords.indexes
                ] = chunk[var].transpose(*chunk_dict[var]["dims"]).values
            elif isinstance(
                chunk[var].variable._data, xr.core.indexing.MemoryCachedArray
            ):
                ...
                raise TypeError(
                    "Dataset data should be either an ndarray or a MemoryCachedArray."
                )
...
def filter_mod_dates(results, from_mod_date=None, to_mod_date=None):
    ...
    """
    if ((from_mod_date is not None) or (to_mod_date is not None)) and (
        "modified_date" in results
    ):
        ...
        if (from_mod_date is not None) and (to_mod_date is not None):
            mod_bool = (mod_dates >= pd.Timestamp(from_mod_date)) & (
                mod_dates <= pd.Timestamp(to_mod_date)
            )
